...
При скролле ко второму экрану (`#showreel`) видео **запускается автоматически**.

- Уход со экрана → пауза
- Возврат → воспроизведение с начала
- Если браузер блокирует звук → кнопка **«Включить звук»**
